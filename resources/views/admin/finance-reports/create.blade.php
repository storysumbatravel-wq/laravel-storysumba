{{-- resources/views/admin/finance-reports/create.blade.php --}}

@extends('layouts.admin')

@section('title', 'Tambah Transaksi - Finance Report')
...
<div class="max-w-3xl mx-auto px-4 py-6">
    ...
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-bold text-[#111111]">Tambah Transaksi Baru</h1>
        <p class="text-xs text-[#484848] mt-1">Income & Fee akan dihitung otomatis berdasarkan input di bawah.</p>
    </div>
    ...
    <form action="{{ route('admin.finance-reports.store') }}" method="POST" class="bg-white rounded-xl border border-[#E1E1E1] p-6">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="item" value="{{ old('item', 'Sewa Hice') }}"
            ...
            <div>
                ...
                <input type="date" name="rental_start_date" value="{{ old('rental_start_date') }}"
                       class="w-full border border-[#E1E1E1] rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#E8B03F]" required>
            </div>
            <div>
                <label class="block text-xs text-[#484848] uppercase tracking-wider mb-2">Tanggal Selesai</label>
                <input type="date" name="rental_end_date" value="{{ old('rental_end_date') }}"
                       class="w-full border border-[#E1E1E1] rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#E8B03F]" required>
            </div>
            ...
            {{-- Rental Price (Input Utama) --}}
            <div>
                <label class="block text-xs text-[#484848] uppercase tracking-wider mb-2">Rental Price (Per Hari) <span class="text-red-500">*</span></label>
                <div class="relative">
                    <span class="absolute left-3 top-2.5 text-gray-400 text-sm">Rp</span>
                    <input type="number" name="rental_price" value="{{ old('rental_price', 0) }}" min="0"
                ...
                <p class="text-xs text-gray-400 mt-1">Income = Rental Price x Duration</p>
            </div>
            ...
            <div>
                ...
                <div class="relative">
                    <span class="absolute left-3 top-2.5 text-gray-400 text-sm">Rp</span>
                    <input type="number" name="expense_bbm" value="{{ old('expense_bbm', 0) }}" min="0"
                           class="w-full border border-[#E1E1E1] rounded-lg pl-10 pr-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#E8B03F]" required>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="relative">
                    <span class="absolute left-3 top-2.5 text-gray-400 text-sm">Rp</span>
                    <input type="number" name="expense_operational" value="{{ old('expense_operational', 0) }}" min="0"
                           class="w-full border border-[#E1E1E1] rounded-lg pl-10 pr-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#E8B03F]" required>
                </div>
                <p class="text-xs text-gray-400 mt-1">Grand Total = Income - (BBM + Op)</p>
            </div>

            {{-- Fee Info --}}
            <div class="bg-blue-50 p-3 rounded-lg md:col-span-2">
                <p class="text-xs text-blue-800"><strong>Catatan Kalkulasi:</strong><br>
                Fee 25% akan dihitung otomatis dari <strong>Grand Total x 25%</strong> setelah data disimpan.</p>
            </div>
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="notes" rows="2"
                          class="w-full border border-[#E1E1E1] rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#E8B03F]">{{ old('notes') }}</textarea>
            </div>
        </div>
        ...
        <div class="flex justify-end gap-3 mt-6 pt-6 border-t border-[#E1E1E1]">
            ...
            <button type="submit"
                    class="px-6 py-2.5 bg-[#1B3B36] text-white rounded-full text-sm font-medium hover:bg-[#2a554e] transition-colors">
                Simpan Transaksi
            </button>
        </div>
    </form>
